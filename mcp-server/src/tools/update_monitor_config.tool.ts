...
export function updateMonitorConfigTool(args: Record<string, any>): McpToolCallResponse {
  try {
    ...
    if (config.alertContacts !== undefined) {
      if (Array.isArray(config.alertContacts)) {
        validatedConfig.alertContacts = config.alertContacts.filter(contact => 
      ...
      if (typeof config.thresholds === 'object') {
        ...
        if (config.thresholds.responseTime !== undefined) {
          if (typeof config.thresholds.responseTime === 'number' && 
              config.thresholds.responseTime > 0) {
        ...
        if (config.thresholds.errorRate !== undefined) {
          if (typeof config.thresholds.errorRate === 'number' && 
              config.thresholds.errorRate >= 0 && 
              config.thresholds.errorRate <= 1) {
        ...
        if (config.thresholds.uptime !== undefined) {
          if (typeof config.thresholds.uptime === 'number' && 
              config.thresholds.uptime >= 0 && 
              config.thresholds.uptime <= 1) {
